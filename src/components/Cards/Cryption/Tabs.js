...
const Tabs = () => {
    ...
    const FileStreamReader = () => {
        ...
        return (blob, offset = 0, length = blob.size) => new Promise((resolve) => {
    ...
    const ptrToArray = (ptr, length) => new Uint8Array(module.HEAPU8.buffer, ptr, length)
    ...
    const decrypt = async () => {
        ...
        const decrypted = ptrToArray(module._decrypt(keyPtr, key.length, encryptedPtr, encrypted.length), encrypted.length - 8);
    ...
    const encrypt = async () => {
        ...
        const encrypted = ptrToArray(module._encrypt(keyPtr, key.length, decryptedPtr, bufferSize), bufferSize);
